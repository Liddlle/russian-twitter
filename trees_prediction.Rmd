...
twitter.coded$clean <- str_replace_all(twitter.coded$clean , "http[[:graph:]]+", "LINK") #заменить mentions
twitter.coded$num_posts <- str_count(twitter.coded$clean, "\n") %>% as.numeric()
twitter.coded$num_posts <- twitter.coded$num_posts + 1 #количество твитов пользователя
...
str_count(twitter.coded$clean, с()) %>% as.numeric()
...
sentiment <- str_count(twitter.coded$clean, rus_sentiments$phrase[rus_sentiments$sentiment=="negative"]) 
