...
mest_ya <- sapply(twitter.coded$all_tweet_2, function(x) str_count(x, " я ")) / n_words
mest_my <- sapply(twitter.coded$all_tweet_2, function(x) str_count(x, " мы ")) / n_words

ya_my <- mest_ya/mest_my

mest_on <- sapply(twitter.coded$all_tweet_2, function(x) str_count(x, " он ")) / n_words
mest_ona <- sapply(twitter.coded$all_tweet_2, function(x) str_count(x, " она ")) / n_words

on_ona <- mest_on/mest_ona
...
dtm.extended <- cbind(looong, dplyr::select(twitter.coded, num_posts:sentiment_found), dtm, mest_ya, mest_my, ya_my, mest_on, mest_ona, on_ona) %>% as.data.frame
